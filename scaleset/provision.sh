...
export SSH_IP=`az vmss list-instance-connection-info -n vmsscluster -g hpccluster | grep instance | awk -F "\"" '{print $4}' | awk -F ":" '{print $1}' | uniq`
export SSH_PORTS=`az vmss list-instance-connection-info -n vmsscluster -g hpccluster | grep instance | awk -F "\"" '{print $4}' | awk -F ":" '{print $2}'`
...
for i in $SSH_PORTS; do
  ...
  ssh -o "StrictHostKeyChecking no" $SSH_IP -p $i -t "sudo hostname node$LOCAL_NAME"
